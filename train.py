...
parser.add_argument('--batch-size', default=8
                    , type=int, metavar='B', help='training batch size')
...
parser.add_argument('--protos', default=10, type=int, help='learnable prototype nums')
...
part_nums = args.protos
dataset = args.dataset
if dataset == 'sysu':
    data_path = '/home/daiyr/Data/SYSU-MM01/'
    log_path = args.log_path + 'sysu_log/'
    test_mode = [1, 2]  # thermal to visible
    pool_dim = 2048 * (part_nums + 1)
elif dataset == 'regdb':
    data_path = '/data/maps/daiyr/RegDB/'
    ...
    data_path = '/home/daiyr/Data/LLCM/'
...
transform_regdb = transforms.Compose([
    ...
    transforms.ToTensor(),
    normalize,
    RandomErasing(probability=args.erasing_p, sl=0.02, sh=0.4, r1=0.3, mean=[0.4914, 0.4822, 0.4465]),
])
...
if dataset == 'sysu':
    ...
    # testing set
    query_img, query_label, query_cam = process_query_llcm(data_path, mode=test_mode[0])
    gall_img, gall_label, gall_cam = process_gallery_llcm(data_path, mode=test_mode[1], trial=0)
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        feat1, out1, loss_ort, feats, outs = net(input1, input2)

        loss_dcs, _, _ = criterion_dcs(feats.float(), labels)

        loss_id = criterion_id(out1, labels.long()) + criterion_id(outs, labels.long())
        ...
        loss_cpm = (criterion_cpm(torch.cat((ft1, ft2), 0), labs) + criterion_cpm(torch.cat((ft1, ft3), 0),
                                                                                  labs)) * args.lambda_1
        ft1, ft2, ft3 = torch.chunk(feats, 3, 0)

        loss_cpm += (criterion_cpm(torch.cat((ft1, ft2), 0), labs) + criterion_cpm(torch.cat((ft1, ft3), 0),
                                                                                   labs)) * args.lambda_1

        loss_ort = loss_ort * args.lambda_2
        loss = loss_id + loss_tri + loss_cpm * 0.5 + loss_ort + loss_dcs
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        dcs_loss.update(loss_dcs.item(), 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                ...
                epoch, batch_idx, len(trainloader), loss_dcs,
...
def test(epoch):
    ...
    gall_feat2 = np.zeros((ngall, pool_dim // 1))
    gall_feat4 = np.zeros((ngall, pool_dim // 1))
    gall_feat6 = np.zeros((ngall, pool_dim // 1))

    gall_feat1 = np.zeros((ngall, pool_dim // (part_nums + 1)))
    gall_feat3 = np.zeros((ngall, pool_dim // (part_nums + 1)))
    gall_feat5 = np.zeros((ngall, pool_dim // (part_nums + 1)))
    ...
    query_feat2 = np.zeros((nquery, pool_dim // 1))
    query_feat4 = np.zeros((nquery, pool_dim // 1))
    query_feat6 = np.zeros((nquery, pool_dim // 1))

    query_feat1 = np.zeros((nquery, pool_dim // (part_nums + 1)))
    query_feat3 = np.zeros((nquery, pool_dim // (part_nums + 1)))
    query_feat5 = np.zeros((nquery, pool_dim // (part_nums + 1)))
    with torch.no_grad():
