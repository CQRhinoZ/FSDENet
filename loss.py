...
class CPMLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # dist_13 = emd_distance_matrix(ft1, ft3)
        # dist_23 = emd_distance_matrix(ft2, ft3)
        # dist_33 = emd_distance_matrix(ft3, ft3)
        # dist_11 = emd_distance_matrix(ft1, ft1)
        #
        dist_14 = pdist_torch(ft1, ft4)
        dist_24 = pdist_torch(ft2, ft4)
        dist_44 = pdist_torch(ft4, ft4)
        dist_22 = pdist_torch(ft2, ft2)
        # dist_14 = emd_distance_matrix(ft1, ft4)
        # dist_24 = emd_distance_matrix(ft2, ft4)
        # dist_44 = emd_distance_matrix(ft4, ft4)
        # dist_22 = emd_distance_matrix(ft2, ft2)

        mask = lbs.expand(n, n).eq(lbs.expand(n, n).t())

        # dist_13 是 vis->vis+ ，dist_14 是 vis->ir+ dist_23 是 ir -> vis+ ，dist_24 是 ir -> ir+
...
class WeightCPMLoss(nn.Module):
    def __init__(self, margin=0.2):
        super(WeightCPMLoss, self).__init__()
        self.margin = margin
        self.ranking_loss = nn.MarginRankingLoss(margin=0.2)
    ...
    def forward(self, inputs, targets):
        ...
        center3 = torch.cat(center3)
        center4 = torch.cat(center4)
        ...
        N = lb1.size(0)
        # mask = lbs.expand(n, n).eq(lbs.expand(n, n).t())
        mask = lb1.expand(N, N).eq(lb1.expand(N, N).t())
...
def emd_distance_matrix(ft1, ft2):
    num_categories = ft1.size(0)
    distance_matrix = torch.zeros((num_categories, num_categories), device=ft1.device)

    for i in range(num_categories):
        for j in range(i + 1, num_categories):
            distance_matrix[i, j] = emd_distance(ft1[i], ft2[j])
...
def emd_distance(distribution1, distribution2):
    distribution1_normalized = distribution1 / distribution1.sum(dim=0, keepdim=True)
    distribution2_normalized = distribution2 / distribution2.sum(dim=0, keepdim=True)

    cumulative_distance = 0.0
    current_distribution1 = torch.zeros_like(distribution1_normalized[0], device=distribution1.device)
    current_distribution2 = torch.zeros_like(distribution2_normalized[0], device=distribution2.device)

    for i in range(distribution1_normalized.size(0)):
        current_distribution1 += distribution1_normalized[i]
        current_distribution2 += distribution2_normalized[i]
        cumulative_distance += torch.abs(current_distribution1 - current_distribution2).sum()

    return cumulative_distance.item()
